...
function planModePlannerUnavailableStatus(mode) {
    if (mode === "goal")
        return "goal_planner_unavailable";
    if (mode === "verify")
        return "verify_planner_unavailable";
    if (mode === "conv")
        return "conv_planner_unavailable";
...
function plannerSourceFromMetadata(metadata) {
    return {
...
async function readInterviewTurns(artifactDir) {
    if (!artifactDir)
        return [];
    try {
        const parsed = JSON.parse(await readFile(join(artifactDir, "interview.json"), "utf8"));
        if (!Array.isArray(parsed.turns))
            return [];
        return parsed.turns.filter((turn) => {
            if (!turn || typeof turn !== "object")
                return false;
            const candidate = turn;
            return ((candidate.role === "planner" || candidate.role === "user") &&
...
async function writeInterviewTurns(artifactDir, parentRunId, turns) {
...
async function commandModePlanRoute(mode, raw, anchor, options = {}) {
    ...
    if (plannerResult.status === "unavailable") {
        ...
        return {
            ...
            user_report: userReport(unavailableStatus, [], [plannerResult.reason], `Retry after the ${planModeLabel(mode)} planner provider is available, or rerun with a sharper request.`),
        };
    }
    ...
    const visibleStatus = result.status === "completed_plan" && plannerResult.status === "draft_ready"
        ? planModeCreatedStatus(mode)
        : planModeNeedsClarificationStatus(mode);
    const interviewTurns = plannerResult.status === "needs_clarification" && providerQuestions.length
        ? [
            ...(options.priorInterviewTurns || []),
            ...providerQuestions.map((question) => ({ role: "planner", content: question, created_at: new Date().toISOString() })),
        ]
        : options.priorInterviewTurns || [];
    const interviewPath = plannerResult.status === "needs_clarification" && interviewTurns.length
        ? await writeInterviewTurns(result.artifact_dir, result.run_id, interviewTurns)
        : undefined;
    const nextAction = visibleStatus === planModeNeedsClarificationStatus(mode)
    ...
    return {
        ...
        user_report: userReport(visibleStatus, [], visibleStatus === planModeNeedsClarificationStatus(mode)
...
export async function runRoute(options) {
    ...
    if (parsed.command === "answer") {
        ...
        const userTurn = { role: "user", content: answer, created_at: new Date().toISOString() };
