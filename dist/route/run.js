...
export async function runRoute(options) {
    ...
    if (parsed.command === "/plan") {
        ...
        return {
            ...
            user_report: userReport(result.status, result.created_files, result.status === "needs_user_decision"
                ? result.plan.ambiguity_questions || ["Plan requires user decision before any execution."]
                : ["Plan mode did not execute; execution still requires a scoped goal approval."], result.status === "needs_user_decision"
    ...
    if (!parsed.rest)
        throw new Error("route /goal requires a goal request JSON path");
    ...
    return {
        ...
        user_report: userReport(result.status, result.created_files, findingRisks(result.findings), result.status === "awaiting_approval"
