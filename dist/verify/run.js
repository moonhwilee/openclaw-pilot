...
async function collectFindings(packet, packetPath) {
    const findings = validateEvidencePacket(packet).map((message) => ({
        code: "schema_invalid",
        message,
        severity: "error",
    }));
    ...
    for (const criterion of requiredCriteria) {
        const supportingEvidence = packet.evidence.filter((item) => item.in_scope && item.supports_claim && item.criteria_ids.includes(criterion.id));
...
function chooseVerdict(findings) {
    ...
    if (findings.some((finding) => finding.code === "evidence_out_of_scope"))
