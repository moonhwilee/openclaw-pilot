...
async function readReceiptRows(stateRoot) {
...
function renderPilotReceiptsDashboard(request, rows, createdAt) {
...
const createArtifact = async ({ request, artifactDir }) => {
...
const createPilotReceiptsDashboard = async ({ request, stateRoot, artifactDir, createdAt }) => {
...
const createReviewNote = async ({ request, artifactDir }) => {
...
function renderRunnerPrompt(request, runId) {
    return [
...
const runCodexSession = async ({ request, artifactDir, runId }) => {
    ...
    if (!envFlag("PILOT_SESSION_RUNNER_ENABLED")) {
        ...
        throw new GoalCapabilityError("PILOT_SESSION_RUNNER_ENABLED is not true; refusing to execute approved session runner.", { artifact_path: resultPath, supporting_artifacts: [promptPath] });
    }
...
export const goalCapabilityRegistry = {
...
export function getGoalCapabilityRunner(capability) {
    return goalCapabilityRegistry[capability];
}
