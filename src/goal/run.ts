...
import { executionPlanCapabilities } from "../execution-plan.ts";
...
function approvalCoversExecutionPlan(request: GoalRequest): boolean {
  if (!request.approval?.approved) return false;
  if (!request.execution_plan) return false;
