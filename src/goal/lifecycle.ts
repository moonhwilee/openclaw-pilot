import type {
  GoalLifecyclePhase,
  GoalLifecycleStep,
  GoalLifecycleSummary,
  GoalRunResult,
  GoalUserStatus,
} from "../types.ts";

function postExecutionVerified(result: GoalRunResult): boolean {
  return result.post_execution_verification?.verdict === "sufficient_evidence";
}

function postConvergenceVerified(result: GoalRunResult): boolean {
  return result.post_convergence_verification?.verdict === "sufficient_evidence";
}

function deriveUserStatus(result: GoalRunResult): GoalUserStatus {
  if (result.status !== "completed") return result.status;
  if (postConvergenceVerified(result)) return "completed_after_convergence";
  if (postExecutionVerified(result)) return "completed_verified";
  return "completed_with_risks";
}

function deriveCurrentPhase(result: GoalRunResult): GoalLifecyclePhase {
  if (result.status === "awaiting_approval") return "approve";
  if (result.post_convergence_verification) return "report";
  if (result.post_execution_convergence) return "reverify";
  if (result.post_execution_verification) return "report";
  if (result.steps.length > 0) return "report";
  return "execute";
}

function deriveNextAction(status: GoalUserStatus, result: GoalRunResult): string {
...
function lifecycleSteps(result: GoalRunResult): GoalLifecycleStep[] {
...
export function buildGoalLifecycleSummary(result: GoalRunResult): GoalLifecycleSummary {
