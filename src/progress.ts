...
type ProgressArtifacts = {
  goalRun?: GoalRunResult;
  convCheckpoint?: ConvCheckpoint;
...
function priorityCountsFromVerification(findings: VerificationFinding[]): string | undefined {
  const actionable = findings.filter((finding) => finding.severity !== "info");
  if (actionable.length === 0) return undefined;
  const counts = Object.fromEntries(priorities.map((priority) => [priority, 0])) as Record<IssuePriority, number>;
  for (const finding of actionable) counts[finding.priority || "P2"] += 1;
  return priorities.map((priority) => `${priority} ${counts[priority]}`).join(", ");
}

function priorityCountsFromConv(findings: ConvFinding[]): string | undefined {
  const open = findings.filter((finding) => finding.status === "open");
  if (open.length === 0) return undefined;
  const counts = Object.fromEntries(priorities.map((priority) => [priority, 0])) as Record<IssuePriority, number>;
  for (const finding of open) counts[finding.priority || "P2"] += 1;
  return priorities.map((priority) => `${priority} ${counts[priority]}`).join(", ");
}

function goalMilestoneLine(result: GoalRunResult): string | undefined {
  ...
  if (!active) return `Goal milestones: ${milestones.length} phases, ${sliceCount} slices`;
  return `Goal phase: ${active.phase_index}/${milestones.length} ${active.goal_phase} (${active.status}, ${sliceCount} slices)`;
}

export function progressLinesForVerification(result: VerificationResult): string[] {
...
export function progressLinesForConv(result: ConvResult, checkpoint?: ConvCheckpoint): string[] {
...
export function progressLinesForGoal(result: GoalRunResult): string[] {
...
export function progressLinesForRecovery(run: PilotRecoveryRunStatus, artifacts: ProgressArtifacts = {}): string[] {
  if (artifacts.goalRun) return progressLinesForGoal(artifacts.goalRun);
  if (artifacts.convResult) return progressLinesForConv(artifacts.convResult, artifacts.convCheckpoint);
  if (artifacts.verification) return progressLinesForVerification(artifacts.verification);
