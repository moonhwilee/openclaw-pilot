...
async function writeConvCheckpoint(
  path: string,
  checkpoint: Omit<ConvCheckpoint, "schema_version" | "updated_at">,
  now: Date,
): Promise<void> {
  await writeJson(path, {
    ...
    ...checkpoint,
  } satisfies ConvCheckpoint);
}

async function validateRequest(request: ConvRequest, requestDir: string): Promise<string[]> {
...
async function finalizeConvRun(options: {
  ...
  checkpointStatus: ConvCheckpoint["status"];
}): Promise<ConvResult> {
  const files = {
...
async function continueConvRun(options: {
  ...
  const receipts: TypedReceipt[] = options.rounds.map((round) => ({
...
export async function resumeConvFromCheckpoint(options: ResumeConvOptions): Promise<ConvResult> {
  const checkpointPath = resolve(options.checkpointPath);
  const checkpoint = JSON.parse(await readFile(checkpointPath, "utf8")) as ConvCheckpoint;
