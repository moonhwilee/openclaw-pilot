...
function versionScopesFromRequest(raw: string): string[] {
  return [...new Set((raw.match(/\bv?\d+\.\d+\.\d+\b/g) || []).map((version) => (version.startsWith("v") ? version : `v${version}`)))];
}

function isImplementationReviewRequest(raw: string): boolean {
  const normalized = raw.toLowerCase();
  const versions = versionScopesFromRequest(raw);
  return (
    versions.length >= 2 ||
    /구현|업데이트|릴리스|release|오버엔지니어링|overengineering|legacy|fallback|no legacy|no fallback|원칙|놓친|잘못된|side effect|사이드/.test(
      normalized,
    )
  );
}

function implementationReviewPlanRequest(raw: string): string {
  const versions = versionScopesFromRequest(raw);
...
async function verifyImplementationReviewPlanRoute(raw: string): Promise<RouteResult> {
  ...
  return {
    ...
    user_report: userReport(
      result.status === "completed_plan" ? "verify_plan_created" : result.status,
      ...
      result.status === "needs_user_decision"
        ...
        : `Review the verification plan. To continue, reply "approve ${shortId}" or cite full run_id ${result.run_id}.`,
      approvalPreview,
      [
...
function artifactShortcutDisabledReport(command: "/verify" | "/conv" | "/goal", path: string): RouteResult {
    ...
    if (command === "cancel" || command === "resume") {
      ...
      return {
        ...
        route: {
          ...
          user_report: userReport(
            status,
            [recoveryCandidateLine(command, latest)],
            [
              command === "cancel"
...
> {
  ...
  if (target.kind === "natural_language") {
    ...
    return {
      ...
      route: targetNeedsRunReport(
        command,
        command === "/verify" ? "verify_needs_evidence" : "conv_needs_anchor_or_plan",
        target.raw,
        command === "/verify"
  ...
  if (parsed.command === "/verify") {
    ...
    return {
      ...
      user_report: userReport(
        ...
        [...target.run.available_artifacts, ...target.run.evidence_pointers],
        [
        ...
        "Collect or cite the exact evidence scope for content review, or use pilot artifact verify <evidence-packet.json> for maintainer-only mechanical checks.",
        undefined,
        [`Run: ${target.run.short_run_id}`, "Verify: content review evidence required"],
      ),
    };
  }
  ...
  if (parsed.command === "/conv") {
    ...
    if (!verification) {
      return targetNeedsRunReport(
        ...
        "Provide a run that already has verification findings, or create a /goal plan to gather the missing review evidence first.",
      );
    }
