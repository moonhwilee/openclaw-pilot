...
function planModeCreatedStatus(mode: PlanMode): string {
  if (mode === "goal") return "goal_plan_created";
  if (mode === "verify") return "verify_plan_created";
  if (mode === "conv") return "conv_plan_created";
  return "plan_created";
}

function planModeNeedsClarificationStatus(mode: PlanMode): string {
  if (mode === "goal") return "goal_needs_clarification";
  if (mode === "verify") return "verify_needs_clarification";
  if (mode === "conv") return "conv_needs_clarification";
...
function commandForPlanMode(mode: PlanMode): "/plan" | "/goal" | "/verify" | "/conv" {
  if (mode === "goal") return "/goal";
  if (mode === "verify") return "/verify";
  if (mode === "conv") return "/conv";
  return "/plan";
}
...
function planModeProgress(mode: PlanMode, anchor?: CommandPlanAnchor): string[] {
  return [
...
async function resolveRunTarget(command: "/verify" | "/conv", raw: string): Promise<
  | { status: "plan"; request: string; anchor?: CommandPlanAnchor; run?: PilotRecoveryRunStatus }
  | { status: "needs_user_decision"; route: RouteResult }
> {
  const stateRoot = defaultStateRoot();
