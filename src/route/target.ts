...
export async function resolveCommandTarget(raw: string): Promise<CommandTarget> {
  const rest = raw.trim();
...
export function findingPriority(finding: VerificationFinding): ConvFinding["priority"] {
  if (finding.priority) return finding.priority;
  return finding.severity === "warning" ? "P3" : "P2";
}

export function buildEvidencePacketFromRun(run: PilotRecoveryRunStatus | PilotRecoveryRunSummary, statement: string): EvidencePacket {
  const artifactPaths = [
...
export async function writeEvidencePacketForRun(
  run: PilotRecoveryRunStatus | PilotRecoveryRunSummary,
  statement: string,
): Promise<string | undefined> {
  const packet = buildEvidencePacketFromRun(run, statement);
  if (packet.evidence.length === 0) return undefined;
  const packetPath = join(run.artifact_dir, "natural-verify-evidence-packet.json");
  await writeJson(packetPath, packet);
  return packetPath;
}

export function buildConvRequestFromVerification(
  run: PilotRecoveryRunStatus | PilotRecoveryRunSummary,
  verification: VerificationResult,
  naturalRequest: string,
): ConvRequest | undefined {
  const findings: ConvFinding[] = verification.findings
  ...
  const anchorPath =
    artifactPaths.find((path) => path.endsWith("verification.json")) ||
...
export async function writeConvRequestFromVerification(
  run: PilotRecoveryRunStatus | PilotRecoveryRunSummary,
  verification: VerificationResult,
  naturalRequest: string,
): Promise<string | undefined> {
