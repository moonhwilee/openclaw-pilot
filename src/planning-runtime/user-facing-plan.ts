...
function modeTitle(mode: PlanMode): string {
  if (mode === "goal") return "Goal Plan";
  if (mode === "verify") return "Verification Plan";
  if (mode === "conv") return "Convergence Plan";
  return "Planning Draft";
}

function compact(values: string[] | undefined, fallback: string[]): string[] {
  const clean = (values || []).map((value) => value.trim()).filter(Boolean);
  return clean.length ? clean : fallback;
}

function userFacingStep(value: string): string {
  return value
...
function phaseApproach(plan: CommonPlanContract): string[] {
  if (!plan.phase_plan?.length) return compact(plan.scope, ["Clarify the requested outcome and keep execution separate from planning."]);
  return plan.phase_plan.map((phase) => `${phase.objective} Check: ${phase.phase_verify}`);
}

function phaseSteps(plan: CommonPlanContract): string[] {
  const phaseSlices = (plan.phase_plan || []).flatMap((phase) =>
    phase.slices.map((slice) => `${slice.objective} Gate: ${slice.convergence_gate}`),
  );
...
function notDoingYet(mode: PlanMode, plan: CommonPlanContract): string[] {
  const common = compact(plan.out_of_scope, ["No execution has been performed yet."]).filter(
    (item) => !/route telegram commands/i.test(item),
  );
  if (mode === "plan") return ["This is planning-only output, not execution approval.", ...common];
  return ["No execution has been performed yet.", "No evidence or verdict has been produced yet.", ...common];
}

function approvalBoundary(mode: PlanMode, plan: CommonPlanContract, executionPlan?: ExecutionPlan): string[] {
  if (mode === "plan") {
    return [
...
function anchorAssumption(anchor?: CommandPlanAnchor): string[] {
  if (!anchor) return [];
...
function verificationPlan(mode: PlanMode, plan: CommonPlanContract): string[] {
