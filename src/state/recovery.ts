...
import type {
  GoalRunResult,
  PilotLineageRecord,
  PilotRecoveryVisibility,
  PilotRecoveryRunStatus,
  PilotRecoveryRunSummary,
  PilotRunIndexEntry,
  RouteCommand,
} from "../types.ts";
import { LINEAGE_INDEX_RELATIVE_PATH } from "./lineage.ts";
import { appendLineageRecord } from "./lineage.ts";
import { readRunIndexEntries, shortRunId } from "./run-index.ts";

export type RecoveryResolution =
  | { status: "found"; run: PilotRecoveryRunStatus }
  | { status: "not_found"; reference: string }
  | { status: "ambiguous"; reference: string; matches: PilotRecoveryRunSummary[] };

export type CancelRecoveryRunResult =
  | { status: "cancelled"; run: PilotRecoveryRunStatus; cancel_artifact: string; previous_status: string }
  | { status: "already_cancelled"; run: PilotRecoveryRunStatus }
  | { status: "not_cancelable"; run: PilotRecoveryRunStatus; reason: string }
  | { status: "not_found"; reference: string }
  | { status: "ambiguous"; reference: string; matches: PilotRecoveryRunSummary[] };

export type ResumeDirectiveResult =
  | { status: "created"; run: PilotRecoveryRunStatus; resume_artifact: string }
  | { status: "not_resumable"; run: PilotRecoveryRunStatus; reason: string };
...
function isLineageRecord(value: unknown): value is PilotLineageRecord {
  if (!value || typeof value !== "object") return false;
  const record = value as Partial<PilotLineageRecord>;
  return (
    record.schema_version === "pilot.lineage.v0" &&
...
async function existingArtifacts(artifactDir: string): Promise<string[]> {
  const names = [
    "goal.json",
    "goal-run.json",
    "plan.md",
...
function isTerminalCompletedStatus(status: string): boolean {
  return (
    status === "completed" ||
...
function recoveryStaleAfterMs(): number {
  const raw = process.env.PILOT_RECOVERY_STALE_AFTER_MS;
  if (!raw) return DEFAULT_RECOVERY_STALE_AFTER_MS;
  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed < 0) return DEFAULT_RECOVERY_STALE_AFTER_MS;
  return parsed;
}

function runAgeMs(createdAt: string, checkedAt: Date): number {
  const created = Date.parse(createdAt);
  if (Number.isNaN(created)) return 0;
  return Math.max(0, checkedAt.getTime() - created);
}

function recoveryVisibilityFor(status: string, createdAt: string): PilotRecoveryVisibility {
  ...
  if (ageMs >= staleAfterMs) {
    return {
      ...
      restart_visible: true,
      hint:
        "Run has not advanced within the recovery freshness window. Inspect status, then choose resume for guidance, cancel the stale run, or start a new /plan or /goal.",
    };
  }
  return {
    status: "fresh",
...
export async function readLineageRecords(stateRoot: string): Promise<PilotLineageRecord[]> {
...
function latestByRun(records: PilotLineageRecord[]): PilotLineageRecord[] {
  const byRun = new Map<string, PilotLineageRecord>();
  for (const record of records) {
    const previous = byRun.get(record.run_id);
    if (!previous || record.created_at >= previous.created_at) byRun.set(record.run_id, record);
  }
  return [...byRun.values()].sort((a, b) => b.created_at.localeCompare(a.created_at));
}

function summaryFromRecord(record: PilotLineageRecord): PilotRecoveryRunSummary {
  return {
...
function summaryFromIndexEntry(entry: PilotRunIndexEntry): PilotRecoveryRunSummary {
  return {
    run_id: entry.run_id,
...
function indexForRun(entries: PilotRunIndexEntry[], runId: string): PilotRunIndexEntry | undefined {
  return entries.filter((entry) => entry.run_id === runId).at(-1);
}

export async function listRecoveryRuns(stateRoot: string, limit = 10): Promise<PilotRecoveryRunSummary[]> {
  const lineage = latestByRun(await readLineageRecords(stateRoot));
  if (lineage.length > 0) return lineage.slice(0, limit).map(summaryFromRecord);
...
export async function resolveRecoveryRun(stateRoot: string, reference: string): Promise<RecoveryResolution> {
  ...
  if (indexMatches.length === 1) {
    ...
    if (indexedRun) {
      ...
      const goalRun = await readJsonIfExists<GoalRunResult>(join(indexedRun.artifact_dir, "goal-run.json"));
      return {
        status: "found",
        run: {
  ...
  if (matches.length === 0) {
    if (indexMatches.length === 0) return { status: "not_found", reference: cleaned };
    if (indexMatches.length > 1) {
      return {
        status: "ambiguous",
        reference: cleaned,
  ...
  const goalRun = await readJsonIfExists<GoalRunResult>(join(latest.artifact_dir, "goal-run.json"));
...
export async function cancelRecoveryRun(
...
): Promise<CancelRecoveryRunResult> {
  ...
  if (run.status === "cancelled") return { status: "already_cancelled", run };
...
export async function isRecoveryRunCancelled(stateRoot: string, reference: string): Promise<boolean> {
  const resolution = await resolveRecoveryRun(stateRoot, reference);
  return resolution.status === "found" && resolution.run.status === "cancelled";
}

export async function createResumeDirective(
  stateRoot: string,
  run: PilotRecoveryRunStatus,
  options: { nextAction: string; risks: string[]; metadata?: Record<string, unknown> },
): Promise<ResumeDirectiveResult> {
    ...
    `${JSON.stringify(
      {
        ...
        note:
          "This directive is a durable handoff for advisory recovery. It does not resume a process or rerun a session.",
        metadata: options.metadata || {},
      },
      null,
      2,
