...
import type { CommonPlanContract, ExecutionPlan, GoalMilestone, GoalPhasePlan, PlanMode, RunStatus } from "../types.ts";

export type CommandPlanAnchor = {
  kind: "run" | "recent";
  reference: string;
...
function requestLooksVagueForMode(request: string, mode: PlanMode): boolean {
  const trimmed = request.trim().toLowerCase();
  if (vagueRequests.has(trimmed)) return true;
  if (mode === "verify" || mode === "conv") return trimmed.length < 4;
...
function normalizeBuildPlanInput(input: string | BuildPlanInput): Required<Pick<BuildPlanInput, "request" | "mode">> & Pick<BuildPlanInput, "anchor"> {
  if (typeof input === "string") return { request: input, mode: "plan" };
  return { request: input.request, mode: input.mode || "plan", anchor: input.anchor };
}

function modeGoal(mode: PlanMode, request: string): string {
  const trimmed = request.trim();
  if (mode === "goal") return trimmed;
  if (mode === "verify") return `Verify: ${trimmed}`;
  if (mode === "conv") return `Converge: ${trimmed}`;
  return trimmed;
}

function modeOutcomeSummary(mode: PlanMode, vague: boolean): string {
  if (vague) return "Pilot needs a sharper target before it can produce an approval-ready plan.";
  if (mode === "verify") {
...
function modeContextSummary(mode: PlanMode, phasePlan: GoalPhasePlan[], anchor?: CommandPlanAnchor): string[] {
  return [
...
function modeScope(mode: PlanMode): string[] {
  if (mode === "verify") {
    return [
...
function modeOutOfScope(mode: PlanMode): string[] {
  const common = [
...
function modeSuccessCriteria(mode: PlanMode): string[] {
  const common = [
...
function modeRisks(mode: PlanMode): string[] {
  if (mode === "verify") {
    return [
...
function modeApprovalRequiredActions(mode: PlanMode): string[] {
  if (mode === "verify") {
    return [
...
function modeVerificationGates(mode: PlanMode): string[] {
  if (mode === "verify") {
    return [
...
function modeNextStep(mode: PlanMode, vague: boolean): string {
  if (vague) return `Answer the ambiguity questions, then rerun /${mode} with a concrete request.`;
  if (mode === "verify") return "Review the verification plan, then approve the plan only if Pilot should collect evidence and run review work.";
  if (mode === "conv") return "Review the convergence plan, then approve the plan only if Pilot should execute convergence work.";
  if (mode === "goal") return "Review the goal plan, then approve the plan only if Pilot should execute the goal.";
  return "Review the plan artifact before any later phase attempts execution.";
}

function modeDetailedTaskBreakdown(mode: PlanMode): string[] {
  if (mode === "verify") {
    return [
...
export function buildPlan(input: string | BuildPlanInput): { status: RunStatus; ambiguityQuestions: string[]; plan: CommonPlanContract } {
...
export function buildExecutionPlan(
  ...
  if (requestLooksVagueForMode(request, mode)) return undefined;
