...
import { defaultConfig, expandHome, loadProjectConfig, projectConfigPath } from "../config.ts";
...
function summarize(checks: DoctorCheck[], strict: boolean): DoctorResult["status"] {
  if (checks.some((check) => check.status === "error")) return "error";
  if (checks.some((check) => check.status === "warning")) return strict ? "error" : "warning";
  return "ok";
}

export async function runDoctor(options: { cwd?: string; strict?: boolean } = {}): Promise<DoctorResult> {
