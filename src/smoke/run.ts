...
import { executionPlanArtifactName, hashExecutionPlan, readExecutionPlan } from "../execution-plan.ts";
import { runGoal } from "../goal/run.ts";
import { runLiveAdapter } from "../live-adapter/run.ts";
...
async function check(name: string, fn: () => Promise<void>): Promise<SmokeCheck> {
  try {
    await fn();
...
export async function runSmoke(): Promise<SmokeResult> {
  ...
  const fixture = (path: string) => resolve(packageRoot, path);
  const checks = await Promise.all([
    check("plan", async () => {
      const result = await runPlan({ request: "Draft a local document strategy plan.", stateRoot });
      if (result.status !== "completed_plan") throw new Error(`unexpected status: ${result.status}`);
    }),
    check("execution_plan_contract", async () => {
      ...
      if (errors.length > 0) throw new Error(`invalid execution plan: ${errors.join("; ")}`);
      if (executionPlan.plan_run_id !== result.run_id) throw new Error("execution plan run id mismatch");
      if (hashExecutionPlan(executionPlan) !== executionPlan.approval_subject_hash) {
        throw new Error("execution plan hash mismatch");
      }
      if (executionPlan.steps.length === 0) throw new Error("execution plan has no steps");
    }),
    check("verify", async () => {
      ...
      if (result.verdict !== "sufficient_evidence") throw new Error(`unexpected verdict: ${result.verdict}`);
    }),
    check("conv", async () => {
    ...
    check("goal_draft", async () => {
      ...
      if (result.status !== "awaiting_approval") throw new Error(`unexpected status: ${result.status}`);
    }),
    check("route_disabled", async () => {
      const result = await runRoute({ input: "/plan Draft a local document strategy plan.", enabled: false });
      if (result.status !== "unavailable") throw new Error(`unexpected status: ${result.status}`);
    }),
    check("live_adapter", async () => {
      ...
      if (result.route.status !== "routed") throw new Error(`unexpected status: ${result.route.status}`);
    }),
  ]);
